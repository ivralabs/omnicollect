"""
OmniCollect Config UI — Windows-compatible version (FastAPI, port 8080).

Differences from main.py:
- Paths relative to script location (Windows-safe with pathlib)
- Opens http://localhost:8080 in the default browser automatically on startup
- Stats path points to edge-agent/stats.json (same location agent writes to)
"""
import json
import os
import base64
...
# ---------------------------------------------------------------------------
# Paths — all relative to edge-agent root
# ---------------------------------------------------------------------------
BASE_DIR = Path(__file__).parent.resolve()
EDGE_DIR = BASE_DIR.parent
...
async def open_browser() -> None:
    """Open the config UI in the default browser after a short delay."""
    def _open() -> None:
        time.sleep(1.5)
...
# ---------------------------------------------------------------------------
# Single-page HTML app (identical to main.py)
# ---------------------------------------------------------------------------
...
(function () {
  ...
  document.getElementById('btn-verify-key').addEventListener('click', async function() {
    ...
    try {
      ...
      setTimeout(function() { goStep('camera'); }, 800);
    } catch (e) {
  ...
  document.getElementById('btn-test-camera').addEventListener('click', async function() {
  ...
  document.getElementById('btn-confirm-camera').addEventListener('click', function() {
  ...
  document.getElementById('btn-reset-zone').addEventListener('click', function() {
    zonePoints = [];
    drawZoneState(canvas);
    show('Tap two points to define the tripwire line.', 'zone-msg', 'ok');
    document.getElementById('zone-btn-row').style.display = 'none';
  ...
  document.getElementById('btn-save-zone').addEventListener('click', async function() {
    ...
    const zone = zonePoints.map(function(p) {
      return [parseFloat((p.x / canvasNaturalW).toFixed(4)), parseFloat((p.y / canvasNaturalH).toFixed(4))];
    });
    setLoading('btn-save-zone', true, 'Saving...');
    try {
      await api('/api/save-zone', { zone: zone });
      show('Zone saved.', 'zone-msg', 'ok');
      setTimeout(function() { loadStatus(); goStep('status'); }, 600);
    } catch (e) {
  ...
  async function loadStatus() {
    try {
      ...
      const config = d.config, stats = d.stats;
      ...
      document.getElementById('status-stats').innerHTML =
        kv('Vehicles today', stats.vehicles_today != null ? stats.vehicles_today : '0') +
  ...
  document.getElementById('btn-redraw').addEventListener('click', function() {
    fetch('/api/status').then(function(r) { return r.json(); }).then(function(d) {
      if (d.config.camera_url) {
        document.getElementById('rtsp-input').value = d.config.camera_url;
      }
      goStep('camera');
    });
  });
  ...
  fetch('/api/status').then(function(r) { return r.json(); }).then(function(d) {
    ...
    if (config.api_key && config.site_name) {
      if (config.zone) { loadStatus(); goStep('status'); }
      else if (config.camera_url) { document.getElementById('rtsp-input').value = config.camera_url; goStep('camera'); }
      else { goStep('camera'); }
    }
  }).catch(function() {});
})();
